 ...
 * 砀山麻游：添加 PASSWORD 登录类型，支持手机号+密码注册/登录
 */
@Service
...
//    private final HashMap<String, User> mobileMap = new HashMap<>();
    ...
    public void loginByOpenId(Login msg, User user) {
        if (msg.getType().equals(LOGIN_TYPE_SMS)) {
            ...
            innerLogin(msg, user);
        } else if (msg.getType().equals(LOGIN_TYPE_PASSWORD)) {
            // 砀山麻游：密码登录/注册
            innerLoginPassword(msg, user);
        }
    }
     ...
     * openId = 手机号, code = 密码
     */
    private void innerLoginPassword(Login msg, User user) {
        asyncDbService.excuete(() -> {
            String mobile = msg.getOpenId();  // 手机号
            String password = msg.getCode();  // 密码
            
            UserDO userDO = userDao.findObject(UserDO.Table.MOBILE, mobile);
            ...
            if (userDO != null) {
                ...
                try {
                    userDO = new UserDO();
                    userDO.setOpenId(mobile);
                    userDO.setPassword(password);  // 砀山麻游：直接使用密码（生产环境应加密）
                    userDO.setName(StringUtils.substring(mobile, 0, 8));
                    ...
                    log.error("重复的注册!" + mobile, ex);
                    userDO = userDao.findObject(UserDO.Table.UNIQUE_OPEN_ID, mobile);
                    if (userDO == null) {
                        log.error("重复注册后检查登录失败!关闭连接" + mobile, ex);
                        user.close();
                    } else {
                        login(user, userDO, msg);
                    }
